---
layout: post
title: "U-Boot 與 Linux 核心編譯、移植與部署自動化實務"
subtitle: "基於 TI AM335x (BeagleBone Black) 平台之現代化交叉編譯與 AI 輔助開發工作流"
categories: [Linux]
date: 2026-06-22
lang: zh-Hant
---
...
本報告探討基於德州儀器（TI）Sitara AM3358 處理器（以 BeagleBone Black 開發板為載體）之嵌入式 Linux 系統建置。內容涵蓋自底層硬體與虛擬化開發主機環境之建構、交叉編譯工具鏈之配置、U-Boot 與 Linux 核心（Kernel）原始碼之編譯，乃至最終開機引導程式與核心映像檔於安全數位卡（SD Card）之部署。此外，本報告針對傳統 Samba 協定配合 PuTTY 終端之工作流，與現代人工智慧輔助整合開發環境（AI-Assisted IDE，以 Antigravity IDE 為代表）進行系統性對比與評估，並詳細說明利用指令行工具 `vmrun` 進行虛擬機無介面（Headless）背景運行管理之實作細節。最後，提出一套透過 `tasks.json` 自動化建構與部署之方案，以期提升嵌入式 BSP（板級支持包）之移植效率。
...
### 2.2 宿主主機雙層控制腳本 (PowerShell / Batch)
...
* **列出目前運行中之虛擬機**：
  ...
  vmrun list
  ```
5另外這邊建立以下兩個腳本檔案，即可實現對虛擬機的一鍵背景控制或選單化管理。

#### 2.2.1 核心控制指令腳本：[`vm_control.ps1`](file:///c:/Users/User/Documents/TechBlog/assets/26_0625/vm_control.ps1)
本腳本支援命令列參數調用，若無參數則會自動開啟一個互動式選單。預設虛擬機對應為 `"Ubuntu"`，其組態檔路徑指向預設的安裝位置。

#### 2.2.2 宿主端便捷啟動批次檔：[`vm_control.bat`](file:///c:/Users/User/Documents/TechBlog/assets/26_0625/vm_control.bat)
...
## 3. BeagleBone Black 啟動流程與四階段引導協定
...
### 3.1 唯讀記憶體引導階段 (ROM Code / Initial Boot Stage)
...
### 3.3 引導加載程式主階段 (U-Boot Main / u-boot.img)
...
### 3.4 核心初始化與根檔案系統掛載階段 (Linux Kernel & Rootfs)
...
### 3.5 清除板載 eMMC 防止開機干擾 (eMMC Content Erasing)
...
## 3. 開發工作流評估：傳統 Samba + PuTTY 與現代 AI IDE 比較

### 3.1 傳統 Samba + PuTTY 工作流架構與瓶頸
...
### 4.2 現代 AI IDE (Antigravity IDE) 工作流之優勢
...
## 5. 多子專案自動化建置與部署 (VS Code Tasks 實務)

基於平行子專案架構（`u-boot`, `kernel`, `output`, `rootfs`），我們在各工作區目錄下配置對應的 `.vscode/tasks.json`，將各階段之編譯與物理燒錄步驟封裝成自動化任務。

### 5.1 U-Boot 引導專案自動化任務：[`u_boot_tasks_example.json`](file:///c:/Users/User/Documents/TechBlog/assets/26_0625/u_boot_tasks_example.json)
...
### 5.2 Linux 核心專案自動化任務：[`kernel_tasks_example.json`](file:///c:/Users/User/Documents/TechBlog/assets/26_0625/kernel_tasks_example.json)
...
### 5.3 物理部署專案自動化任務：[`output_tasks_example.json`](file:///c:/Users/User/Documents/TechBlog/assets/26_0625/output_tasks_example.json)
